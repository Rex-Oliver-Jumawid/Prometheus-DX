...
Manual comparison against Figma node `11:2301` at desktop, medium, and narrow/mobile viewports remains pending; the local API build exit status is now confirmed.

Next: perform the Phase 7 manual Notifications acceptance checklist in the phase journal. Compare sidebar placement, unread-count treatment, inbox states, responsive layout, accessible controls, and read persistence with Figma. The Figma mockup's Mentions and Projects tabs are outside the current All/Unread scope, as documented in the phase journal.
