...
export function createSupabaseAuthClient(): SupabaseAuthClient | null {
  ...
  return createClient(serverEnvironment.supabaseUrl, serverEnvironment.supabaseAnonKey, {
    auth: {
...
export class AuthService {
  constructor(
    ...
    @Inject(SUPABASE_AUTH_CLIENT) private readonly supabase: SupabaseAuthClient | null,
  ) {}
  ...
  async resolveActiveMember(accessToken: string): Promise<Member> {
    ...
    try {
      let member = await this.prisma.member.findUnique({ where: { authUserId: user.id } });
      ...
      if (!member && normalizedEmail && user.email_confirmed_at) {
        ...
        if (emailMember && !emailMember.authUserId && emailMember.status !== 'DEACTIVATED') {
          ...
          if (linked.count === 1) {
            member = await this.prisma.member.findUnique({ where: { id: emailMember.id } });
          }
        }
      }

      if (!member || member.status !== 'ACTIVE') {
        throw new ForbiddenException('This account does not have access to Prometheus.');
      }
      ...
      throw new ServiceUnavailableException('Workspace authorization is temporarily unavailable.');
    }
  }
}
