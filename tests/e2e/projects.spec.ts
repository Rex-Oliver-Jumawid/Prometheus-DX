...
test.beforeAll(async () => {
  ...
  const [firstDepartment, secondDepartment] = await Promise.all([
    prisma.department.create({
      data: { name: `${runId} Design`, shortLabel: 'P3DES' },
    }),
    prisma.department.create({
      data: { name: `${runId} Delivery`, shortLabel: 'P3DEL' },
    }),
  ]);
  ...
  const unrelatedProject = await prisma.project.create({
    data: {
      ...
      description:
        'A real Project visible to an active Member without authority.',
  ...
  const leadProject = await prisma.project.create({
    data: {
      ...
      description:
        'A real Project for exercising Project Lead status controls.',
      createdByMemberId: lead.id,
...
test.afterAll(async () => {
  ...
  if (fixtureLeadId)
    await prisma.member.delete({ where: { id: fixtureLeadId } });
  if (fixtureDepartmentIds.length)
    await prisma.department.deleteMany({
      where: { id: { in: fixtureDepartmentIds } },
    });
  await prisma.$disconnect();
});
...
test('an active Member creates a Project with another active Lead and multiple Departments', async ({
  page,
}) => {
  test.skip(!hasCredentials, 'Requires E2E member credentials.');
  const errors: string[] = [];
  ...
  const creator = await prisma.member.findFirstOrThrow({
    where: {
      email: { equals: process.env.E2E_MEMBER_EMAIL!, mode: 'insensitive' },
    },
  });
  ...
  await page
    ...
    .fill('Created through the focused Phase 3 browser acceptance path.');
  await page.getByLabel('Project Lead').selectOption(creator.id);
  await page.getByLabel('Project Lead').selectOption(fixtureLeadId!);
  for (const departmentId of fixtureDepartmentIds)
    await page.locator(`input[value="${departmentId}"]`).check();
  ...
  expect(
    (
      request.postDataJSON() as { departmentIds: string[] }
    ).departmentIds.sort(),
  ).toEqual([...fixtureDepartmentIds].sort());
  await expect(page.getByRole('dialog', { name: 'Add Project' })).toHaveCount(
    0,
  );
  ...
  expect(project.departments.map((item) => item.departmentId).sort()).toEqual(
...
test('an unrelated active Member can open a Project but cannot change its status', async ({
  page,
}) => {
  test.skip(!hasCredentials, 'Requires E2E member credentials.');
  await signIn(page);
  ...
  await expect(ownershipCard.locator('dt + dd').first()).toContainText(
    'Phase Three Alternate Lead',
  );
  await expect(page.getByLabel('Project status')).toHaveCount(0);
  expect(await apiStatusUpdate(page, unrelatedProjectId!, 'IN_PROGRESS')).toBe(
    403,
  );
});

test('a creator and Administrator without the Lead relationship receive no status authority', async ({
  page,
}) => {
  test.skip(!hasCredentials, 'Requires E2E member credentials.');
...
test('the Project Lead changes status through the direct Project route and it persists', async ({
  page,
}) => {
  test.skip(!hasCredentials, 'Requires E2E member credentials.');
  await signIn(page);
  ...
  await expect(statusControl.locator('option[value="ARCHIVED"]')).toHaveCount(
    0,
  );
  const inProgressResponse = page.waitForResponse(
    (response) =>
      response.url().endsWith(`/api/projects/${leadProjectId}/status`) &&
      response.request().method() === 'PATCH',
  );
  await statusControl.selectOption('IN_PROGRESS');
  await expect(statusControl).toHaveValue('IN_PROGRESS');
  expect((await inProgressResponse).status()).toBe(200);
  ...
  expect(
    project.statusHistory.map((item) => ({
      ...
      to: item.toStatus,
      source: item.changeSource,
      by: item.changedByMemberId,
    })),
  ).toEqual([
    {
      from: 'PLANNING',
...
test('a nonexistent or invalid Project route shows a controlled not-found state', async ({
  page,
}) => {
  test.skip(!hasCredentials, 'Requires E2E member credentials.');
  await signIn(page);
