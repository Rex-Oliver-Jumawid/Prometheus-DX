...
Configure My Schedule opens a draft editor above the merged team calendar.
Members can generate initial blocks, select and adjust their own blocks, drag or resize them, and toggle rest days.
The numeric rest-day count and calendar day-header selections remain synchronized, including an intentional zero selection.
Changing the count may remove conflicting planned blocks; the editor reports any removals so the member can redistribute their hours.
Rest-day weekdays persist explicitly alongside the weekly target and recurring blocks, so an empty workday does not become a rest day after reload.
