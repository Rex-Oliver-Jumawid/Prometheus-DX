...
export function AccessDeniedPage() {
  const auth = useAuth();
  if (auth.session === undefined || (auth.session && auth.memberPending))
    ...
    return (
      <main className="denied-page">
        <section className="denied-card">
          ...
          <p className="auth-eyebrow">WORKSPACE CHECK</p>
          <h1>Access could not be verified</h1>
          <p>
            Prometheus did not load protected content. Check your connection and
            try again.
          </p>
  ...
  return (
    <main className="denied-page">
      <section className="denied-card">
        ...
        <h1>This account can’t enter Prometheus</h1>
        <p>
          Your sign-in succeeded, but this account is not currently authorized
          for the workspace. Contact your administrator if you believe this is a
          mistake.
        </p>
        <button
          ...
          type="button"
          onClick={() =>
            void auth.signOut().then(() => location.assign('/login'))
          }
        >
